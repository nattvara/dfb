...
func (gui *ProgressGUI) updateLayout() {
	...
	scroll := fyne.NewContainerWithLayout(layout.NewFixedGridLayout(fyne.NewSize(1400, 400)))
	...
	for _, file := range gui.currentDomain.StatusLines {
		file.Resize(fyne.NewSize(1400, 10))
	...
	bottom := fyne.NewContainerWithLayout(
		layout.NewFixedGridLayout(fyne.NewSize(1400, 40)),
	...
	content.Resize(fyne.NewSize(1400, 475))
